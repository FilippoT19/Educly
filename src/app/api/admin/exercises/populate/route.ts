...
// Batch populate all exercises missing answers for a given subject
export async function POST(request: NextRequest) {
  ...
  // Get exercises with empty answers
  ...
  if (!exercises || exercises.length === 0) {
    return NextResponse.json({ processed: 0, message: "Nessun esercizio da popolare." });
  }
  ...
  for (const ex of exercises) {
    try {
      ...
      errors.push(`${ex.id}: ${err instanceof Error ? err.message : "errore"}`);
    }
  }
